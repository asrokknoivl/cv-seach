...
Ce TP est à réaliser de préférence sous Linux (accès facile à `mvn` et `git`
en ligne de commande). Il est vivement recommandé de faire les manipulations dans un terminal (éventuellement le terminal intégré de votre IDE préféré). Une fois les bases de `mvn` et `git` bien maîtrisées, vous aurez sans doute envie d'utiliser les outils graphiques et les raccourcis clavier intégrés à votre éditeur de texte ou IDE.
...
Pour les TPs suivants, il est quasi-indispensable d'utiliser un IDE (VSCode, IntelliJ, Eclipse, ...).
...
puis valider en indiquant le numéro de la demande `\#1` (en remplaçant `1` par
le vrai numéro de l'issue si besoin) dans le message de commit :
...
- https://forge.univ-lyon1.fr/matthieu.moy/mif01-2021, qui est le
